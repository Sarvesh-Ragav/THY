...
import { findUserById, createOtpChallenge, createRefreshSession, revokeRefreshSession, rotateRefreshSession, verifyOtp } from '../services/auth.service.js';
...
import { normalizePhoneNumber, requestOtpSchema, verifyOtpSchema } from '../validators/auth.schemas.js';

const refreshCookieName = 'thy_refresh_token';
const refreshCookie = { httpOnly: true, secure: env.NODE_ENV === 'production', sameSite: 'lax' as const, path: '/api/v1/auth', maxAge: env.REFRESH_TOKEN_TTL_DAYS * 86_400_000 };
const requestMetadata = (request: Parameters<RequestHandler>[0]) => ({ requestIp: request.ip, userAgent: request.get('user-agent') });
...
function validatedPhoneNumber(value: string): string {
  ...
  catch (error) { throw new ApiError(400, error instanceof Error ? error.message : 'Invalid phone number.', 'VALIDATION_ERROR'); }
}

async function issueTokens(user: { id: string; phoneNumber: string }, request: Parameters<RequestHandler>[0], response: Parameters<RequestHandler>[1], rotateFrom?: string): Promise<string> {
  const payload = { userId: user.id, phoneNumber: user.phoneNumber };
  const refreshToken = createRefreshToken(payload);
  const metadata = requestMetadata(request);
  if (rotateFrom) await rotateRefreshSession(user.id, hashValue(rotateFrom), hashValue(refreshToken), refreshExpiry(), metadata.requestIp, metadata.userAgent);
  else await createRefreshSession(user.id, hashValue(refreshToken), refreshExpiry(), metadata.requestIp, metadata.userAgent);
...
export const requestOtp: RequestHandler = async (request, response, next) => {
  try {
    ...
    response.status(202).json({ success: true, message: 'Verification code sent.', data: { challengeId: result.challengeId, expiresAt: result.expiresAt.toISOString(), ...(result.mockOtp ? { mockOtp: result.mockOtp } : {}) } });
...
export const refresh: RequestHandler = async (request, response, next) => {
  try {
    ...
    if (!currentToken) return response.status(401).json({ success: false, error: { code: 'REFRESH_INVALID', message: 'Refresh session is missing.' } });
