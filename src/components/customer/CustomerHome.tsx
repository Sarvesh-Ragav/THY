    ...
    <>
      <section className="relative min-h-[78vh] overflow-hidden">
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 min-h-[78vh] flex items-end pb-16 pointer-events-none">
          <div className="max-w-xl text-[#F3EEE4] pointer-events-auto">
            <h1 className="text-[clamp(3rem,6.5vw,6.4rem)] leading-[0.9]" style={{ fontFamily: 'var(--font-cormorant), serif' }}>
              Stitch Your Desire
            </h1>
            <p className="mt-4 text-sm md:text-base tracking-[0.04em]">
              Your Fabric. Your Style. Your Tailor.
            </p>
            <Link
              href="/stitch-your-outfit"
              className="hero-leather-btn inline-flex mt-8 px-7 py-3.5 text-[11px] font-semibold uppercase tracking-[0.18em] text-[#2C2418]"
            >
              Stitch Your Outfit
            </Link>
          </div>
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-14 space-y-16">
        ...
        <div>
          ...
          <div className="mt-6 grid grid-cols-2 md:grid-cols-4 gap-4">
            {trending.map((design) => (
              <Link key={design.id} href="/explore" className="overflow-hidden border border-[#2C2418]/10">
                <img src={design.image} alt="" className="h-40 w-full object-cover" />
                <p className="p-3 text-sm">{design.title}</p>
              </Link>
            ))}
          </div>
          <Link href="/explore" className="inline-block mt-4 text-[11px] uppercase tracking-[0.18em] border-b border-[#2C2418]">
            Explore Relevant Trending Designs
          </Link>
        </div>

        <div className="border border-[#2C2418]/10 p-6 bg-white/40">
          <h2 className="text-3xl" style={{ fontFamily: 'var(--font-cormorant), serif' }}>
            Continue Your Style Journey
          </h2>
          {activity ? (
            <div className="mt-4 flex flex-wrap gap-4 text-sm">
              <Link href="/my-designs" className="border-b border-[#2C2418]">My Designs</Link>
              <Link href="/my-orders" className="border-b border-[#2C2418]">My Orders</Link>
              <Link href="/my-measurements" className="border-b border-[#2C2418]">My Measurements</Link>
            </div>
          ) : (
            <div className="mt-4">
              <p className="text-sm text-[#5C5146]">Ready to Stitch Something You Love?</p>
              <p className="text-sm text-[#8A7D70] mt-1">Start your style journey with THY.</p>
              <Link
                href="/stitch-your-outfit"
                className="hero-leather-btn inline-flex mt-5 px-6 py-3 text-[11px] uppercase tracking-[0.18em] text-[#2C2418]"
              >
                Stitch Your Outfit
              </Link>
            </div>
