    ...
    <main className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8 md:py-12">
      ...
      <section className="mt-4 thy-card p-5 sm:p-7">
        ...
        <div className="mt-4 flex flex-wrap gap-2">
          {tailor.languages.map((language) => (
            <span key={language} className="px-3 min-h-8 inline-flex items-center text-xs border border-thy-ink/10 bg-thy-mist text-thy-deep">
              {language}
            </span>
          ))}
        </div>
      ...
      <section className="mt-4 thy-card p-5 sm:p-7">
        ...
        <div className="mt-4 flex flex-wrap gap-2">
          {tailor.skills.map((skill) => (
            <span key={skill} className="px-3 min-h-9 inline-flex items-center text-sm border border-thy-brand/30 bg-thy-mist text-thy-deep">
              {skill}
            </span>
          ))}
        </div>
      ...
      <section className="mt-4 thy-card p-5 sm:p-7">
        ...
        <div className="mt-4 grid grid-cols-2 sm:grid-cols-3 gap-3">
          {tailor.portfolio.map((image) => (
            <img key={image} src={image} alt="" className="h-36 sm:h-40 w-full object-cover border border-thy-ink/10" />
          ))}
        </div>
      </section>
