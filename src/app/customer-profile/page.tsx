    ...
    <div className="thy-app-glow min-h-dvh bg-transparent text-thy-ink pb-20 md:pb-12">
      ...
      <main className="max-w-6xl mx-auto px-0 sm:px-4 py-0 sm:py-6">
        ...
        <div className="bg-white sm:rounded-2xl shadow-sm border border-slate-200/80 overflow-hidden mb-6">
          ...
          <div className="h-40 sm:h-56 relative bg-slate-800">
            ...
            <div className="absolute top-4 right-4 bg-[#F3E8D6] text-[#5C1A24] border border-[#5C1A24]/30 px-3 py-1.5 rounded-full text-xs font-bold flex items-center gap-1.5 shadow-md">
              <Sparkles className="w-3.5 h-3.5 fill-[#5C1A24]" />
          ...
          <div className="relative px-4 sm:px-8 pb-6">
            <div className="flex flex-col sm:flex-row sm:items-end justify-between -mt-14 sm:-mt-16 mb-5 gap-4">
              ...
              <div className="flex flex-col sm:flex-row items-center sm:items-end gap-4 text-center sm:text-left">
                ...
                <div className="pt-2 sm:pt-0">
                  ...
                  <div className="flex flex-wrap items-center justify-center sm:justify-start gap-3 mt-2 text-xs text-slate-500">
                    ...
                    <span>•</span>
                    <span className="flex items-center gap-1">
                      <Calendar className="w-3.5 h-3.5 text-[#5C1A24]" />
              ...
              <div className="flex items-center justify-center sm:justify-end gap-2.5 mt-2 sm:mt-0">
                <button 
                  ...
                  className="flex-1 sm:flex-initial px-4 py-2.5 rounded-xl bg-[#5C1A24] hover:bg-[#4A1520] text-white text-xs sm:text-sm font-semibold transition-all shadow-sm flex items-center justify-center gap-1.5 active:scale-95"
                >
                ...
                <button 
                  ...
                  className="flex-1 sm:flex-initial px-4 py-2.5 rounded-xl bg-[#F3E8D6] hover:bg-[#EFE4D2] text-[#5C1A24] border border-[#5C1A24]/30 text-xs sm:text-sm font-semibold transition-all flex items-center justify-center gap-1.5"
                >
            ...
            {/* Quick Stats Grid */}
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 sm:gap-4 mt-6 p-3 sm:p-4 rounded-xl bg-[#F3E8D6]/60 border border-[#5C1A24]/20 text-center">
              <div className="p-1">
                <div className="text-xl sm:text-2xl font-bold text-[#5C1A24]">{CUSTOMER_DATA.stats.activeOrders}</div>
                <div className="text-xs text-slate-600 font-medium mt-0.5">Active Orders</div>
              </div>
              <div className="p-1 border-l border-slate-200 sm:border-x sm:border-[#5C1A24]/20">
                <div className="text-xl sm:text-2xl font-bold text-[#5C1A24]">{CUSTOMER_DATA.stats.completedDesigns}</div>
              ...
              <div className="p-1 border-t sm:border-t-0 border-slate-200">
                <div className="text-xl sm:text-2xl font-bold text-[#5C1A24]">{CUSTOMER_DATA.stats.savedTailors}</div>
              ...
              <div className="p-1 border-t border-l sm:border-t-0 border-slate-200">
                <div className="text-xl sm:text-2xl font-bold text-[#5C1A24]">{CUSTOMER_DATA.stats.stitchPoints}</div>
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between pb-4 border-b border-slate-100 gap-3">
            <div>
              <div className="flex items-center gap-2">
                <Ruler className="w-5 h-5 text-[#5C1A24]" />
                ...
                <button
                  ...
                  className={`px-3 py-1.5 rounded-lg text-xs font-semibold whitespace-nowrap transition-all border ${
                    selectedProfileId === prof.id
                      ? "bg-[#5C1A24] text-white border-[#5C1A24] shadow-sm"
          ...
          <div className="mt-4">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              {Object.entries(currentMeasurementObj.data).map(([key, value]) => {
                ...
                return (
                  <div key={key} className="p-3 rounded-xl bg-[#F3E8D6]/40 border border-[#5C1A24]/15 flex flex-col justify-between">
            ...
            <div className="mt-4 flex flex-wrap items-center justify-between gap-3 pt-3 border-t border-slate-100 text-xs">
              <span className="text-slate-500 flex items-center gap-1">
                <CheckCircle2 className="w-4 h-4 text-thy-burgundy" /> Standardised fit values verified by THY Master Tailors
              </span>
              <div className="flex items-center gap-3">
                <button className="text-[#5C1A24] hover:underline font-semibold flex items-center gap-1">
              ...
              return (
                <button
                  ...
                  className={`flex-1 min-w-[140px] sm:min-w-0 py-3.5 px-4 flex items-center justify-center gap-2 text-xs sm:text-sm font-semibold transition-colors border-b-2 whitespace-nowrap ${
                    isActive 
                      ? 'border-[#5C1A24] text-[#5C1A24] bg-[#F3E8D6]/30' 
                      : 'border-transparent text-slate-500 hover:text-slate-800 hover:bg-slate-50'
                  }`}
                >
                  <Icon className={`w-4 h-4 ${isActive ? 'text-[#5C1A24]' : 'text-slate-400'}`} />
                  {tab.label}
                </button>
              );
            })}
          ...
          {activeTab === 'orders' && (
            <div className="space-y-4">
              {CUSTOMER_DATA.orders.map((order) => (
                <div key={order.id} className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-4 sm:p-5 hover:border-[#5C1A24]/40 transition-all">
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between pb-4 border-b border-slate-100 gap-2">
                    <div className="flex items-center gap-3">
                      <span className="text-xs font-bold bg-[#F3E8D6] text-[#5C1A24] px-2.5 py-1 rounded-md border border-[#5C1A24]/20">
                        {order.id}
                      </span>
                      <div>
                    ...
                    <div className="flex items-center gap-2">
                      <span className={`text-xs font-semibold px-3 py-1 rounded-full border ${
                        order.status === 'Delivered' 
                          ? 'bg-thy-mist text-thy-burgundy border-thy-burgundy/20' 
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4 my-4">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div className="text-xs space-y-1">
                        ...
                        <p className="text-[#5C1A24] font-medium">{order.price}</p>
                      </div>
                    </div>
                    ...
                    <div className="flex flex-col justify-center text-xs space-y-1">
                      ...
                      <span className="text-[#5C1A24]">Profile: {order.measurementsUsed}</span>
                    </div>
                  </div>
                  ...
                  {order.status !== 'Delivered' && (
                    <div className="bg-slate-50 p-3 rounded-xl border border-slate-100 my-3">
                      <div className="flex justify-between text-[11px] font-semibold text-slate-600 mb-1.5">
                        ...
                        <span className="text-[#5C1A24]">Step {order.statusStep} of 5</span>
                      </div>
                      <div className="w-full bg-slate-200 h-2 rounded-full overflow-hidden">
                        <div 
                          className="bg-[#5C1A24] h-full transition-all duration-500 rounded-full" 
                  ...
                  <div className="flex items-center justify-end gap-2 pt-3 border-t border-slate-100">
                    ...
                    <button className="px-3.5 py-1.5 rounded-lg bg-[#5C1A24] hover:bg-[#4A1520] text-white text-xs font-semibold flex items-center gap-1">
                ...
                <div key={tailor.id} className="bg-white rounded-xl overflow-hidden border border-slate-200/80 shadow-sm hover:shadow-md transition-all flex flex-col justify-between">
                  <div>
                    ...
                    <div className="p-4 pt-0 relative">
                      ...
                      <p className="text-xs text-[#5C1A24] font-medium mt-1">{tailor.specialty}</p>
                  ...
                  <div className="p-4 pt-0 flex items-center gap-2">
                    <button className="flex-1 py-2 rounded-lg bg-[#5C1A24] hover:bg-[#4A1520] text-white text-xs font-semibold flex items-center justify-center gap-1">
            ...
            <div>
              <div className="flex items-center justify-between mb-4">
                ...
                <button 
                  ...
                  className="px-3.5 py-2 rounded-xl bg-[#5C1A24] text-white text-xs font-semibold flex items-center gap-1 hover:bg-[#4A1520]"
                >
                  ...
                  <div key={item.id} className="bg-white rounded-xl overflow-hidden border border-slate-200/80 shadow-sm flex flex-col justify-between">
                    <div>
                      ...
                      <div className="p-4">
                        ...
                        <div className="mt-3 text-[11px] bg-[#F3E8D6] text-[#5C1A24] px-2.5 py-1 rounded border border-[#5C1A24]/20 inline-block font-medium">
              ...
              {CUSTOMER_DATA.reviewsGiven.map((rev) => (
                <div key={rev.id} className="p-4 rounded-xl border border-slate-100 bg-slate-50/50 flex flex-col sm:flex-row gap-4">
                  ...
                  <div className="space-y-1 flex-1">
                    ...
                    <p className="text-xs font-medium text-[#5C1A24]">Order: {rev.garment}</p>
      ...
      {selectedOrderModal && (
        <div className="fixed inset-0 z-50 bg-slate-900/80 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl max-w-lg w-full p-6 relative shadow-2xl space-y-4">
            ...
            <div className="flex items-center gap-2">
              <FileText className="w-5 h-5 text-[#5C1A24]" />
            ...
            <div className="space-y-2 text-xs border-y border-slate-100 py-3">
              ...
              <div className="flex justify-between"><span className="text-slate-500">Total Price:</span><span className="font-bold text-[#5C1A24]">{selectedOrderModal.price}</span></div>
            </div>

            <button 
              onClick={() => setSelectedOrderModal(null)}
              className="w-full py-2.5 rounded-xl bg-[#5C1A24] text-white text-xs font-bold"
            >
              Close Summary
            </button>
          </div>
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-slate-200 p-3 sm:hidden z-40 flex items-center justify-between gap-3 shadow-lg">
        <button 
          ...
          className="flex-1 py-2.5 rounded-xl bg-[#5C1A24] text-white text-xs font-bold flex items-center justify-center gap-1.5 shadow-sm active:scale-95 transition-all"
        >
          <ShoppingBag className="w-4 h-4" /> My Orders
        </button>
        <button 
          onClick={() => setActiveTab('wishlist')}
          className="flex-1 py-2.5 rounded-xl bg-[#F3E8D6] text-[#5C1A24] border border-[#5C1A24]/30 text-xs font-bold flex items-center justify-center gap-1.5"
        >
